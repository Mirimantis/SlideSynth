...
### Viewport & canvas
- [x] **13.3 New composition with Lock Rail on starts at 0:00 on the rail** *(S, bug)*
  ...
  *Shipped: the boot block hardcoded `offsetX = 0`, which puts beat 0 at the left edge while the rail sits at mid-canvas. Now boots through `scrollViewportToBeat(viewport, 0, ...)` when Lock Rail is on, and toggling Lock Rail on does the same **only** on an empty composition — with content on the canvas, yanking the view would be worse than leaving it, and playback re-centres on the playhead anyway. The negative `offsetX` this needs was already supported by `minPanOffsetX`.*
